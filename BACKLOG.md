...
- [ ] Przywrócić odtwarzalny build `app/FlippChill_Kalkulator.html` — śledzone w #7. Release gate ma dwa niezależne artefakty: 7A canonical BEST56 i 7B historyczny BEST40. `scripts/stage_canonical_app.py` ma runtime-verified staging exact BEST56 SHA-256 `3bb0756f6d3e55a0f5eeb35baec1489be4862ddddabb93c9df97acd9f4044e92`; iteracja 33 dodała deterministyczne `--auto` discovery, które przyjmuje wyłącznie ten exact hash. `tests/check_stage_canonical_app.py` obejmuje exact match, odrzucenie near-match, brak matcha i deterministyczny wybór; workflow #226 potwierdził PASS dla rozszerzonego gate przed `Static application checks`. `P0-7A-CANONICAL-APP` pozostaje READY — PRIMARY ma uruchomić `python scripts/stage_canonical_app.py --auto`, zapisać canonical app i przejść `tests/check_app.py`. `P0-7B-FROZEN-BEST40` pozostaje BLOCKED do exact SHA-256 `c04106fe884d32dc257d852b320f2e145a93f80e5615409dc5fac17f5b171708`.
