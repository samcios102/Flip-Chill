...
- [ ] Przywrócić odtwarzalny build `app/FlippChill_Kalkulator.html` — śledzone w #7. Release gate ma dwa niezależne artefakty: 7A canonical BEST56 i 7B historyczny BEST40. `scripts/stage_canonical_app.py` ma runtime-verified staging exact BEST56 SHA-256 `3bb0756f6d3e55a0f5eeb35baec1489be4862ddddabb93c9df97acd9f4044e92`; iteracja 34 dodała przenośny kontrakt `FLIPPCHILL_ARTIFACT_ROOTS`, dzięki któremu różne runtime'y agentów mogą wskazać montowany/pobrany katalog bez hardkodowania ścieżki. Exact-hash-only pozostaje obowiązkowe. Workflow #239 potwierdził PASS dla rozszerzonego gate przed `Static application checks`. `P0-7A-CANONICAL-APP` pozostaje READY — PRIMARY ma ustawić `FLIPPCHILL_ARTIFACT_ROOTS` jeśli potrzeba, uruchomić `python scripts/stage_canonical_app.py --auto`, zapisać canonical app i przejść `tests/check_app.py`. `P0-7B-FROZEN-BEST40` pozostaje BLOCKED do exact SHA-256 `c04106fe884d32dc257d852b320f2e145a93f80e5615409dc5fac17f5b171708`.
...
- [ ] P1 #12 — uruchomić lokalny `scripts/agent_dispatch.py --watch` z rzeczywistą komendą OpenCode/bota i potwierdzić pełny cykl `RUN_FIX → guard → claim → wykonanie → test → handoff`. Claim/lock, failure recovery, mutex READY→CLAIMED, bezpieczne odzyskanie starego osieroconego mutexa, dependency guard, świeżość handoffu oraz bezpośrednia integracja runtime guardu z dispatcherem są deterministycznie wymuszane przez CI. Iteracja 35 dodała kanoniczne `sync_contract.required_read_order` dla 3 botów, w tym obowiązkowy `sync/CRM_SYNC.md` przed BACKLOG; `tests/check_ai_sync_protocol.py` blokuje drift, a workflow #257 potwierdził `Verify AI sync dispatch protocol = PASS`. Pozostał pełny lokalny runtime z rzeczywistym `FLIPPCHILL_BOT_COMMAND`.
