...
- [ ] Przywrócić odtwarzalny build `app/FlippChill_Kalkulator.html` — śledzone w #7. Release gate ma dwa niezależne artefakty: 7A canonical BEST56 i 7B historyczny BEST40. Exact BEST56 SHA-256 `3bb0756f6d3e55a0f5eeb35baec1489be4862ddddabb93c9df97acd9f4044e92` jest runtime-verified. Iteracja 36 dodała `scripts/materialize_canonical_app.py` + `tests/check_materialize_canonical_app.py` i gate CI dla deterministycznego gzip+base64 materializera z exact-SHA oraz atomowym zapisem. Repozytoryjny source payload `artifacts/best56/` pozostaje do bezpiecznego transferu; nie zapisano częściowego artefaktu. `P0-7A-CANONICAL-APP` pozostaje READY dla PRIMARY: preferowany repo materializer, a bezpieczny fallback to `FLIPPCHILL_ARTIFACT_ROOTS` + `python scripts/stage_canonical_app.py --auto`. `P0-7B-FROZEN-BEST40` pozostaje BLOCKED do exact SHA-256 `c04106fe884d32dc257d852b320f2e145a93f80e5615409dc5fac17f5b171708`.
...
- [ ] P1 #12 — uruchomić lokalny `scripts/agent_dispatch.py --watch` z rzeczywistą komendą OpenCode/bota i potwierdzić pełny cykl `RUN_FIX → guard → claim → wykonanie → test → handoff`. Claim/lock, failure recovery, mutex READY→CLAIMED, bezpieczne odzyskanie starego osieroconego mutexa, dependency guard, świeżość handoffu oraz bezpośrednia integracja runtime guardu z dispatcherem są deterministycznie wymuszane przez CI. Iteracja 37 zsynchronizowała root `OPENCODE.md` z kanonicznym `sync_contract.required_read_order`, w tym obowiązkowym `AI_SYNC/PROTOCOL.md` jako krokiem 2; `tests/check_ai_sync_protocol.py` blokuje teraz drift zarówno `AI_SYNC/PROTOCOL.md`, jak i `OPENCODE.md`. Workflow #282 potwierdził `Verify AI sync dispatch protocol = PASS`. Pozostał pełny lokalny runtime z rzeczywistym `FLIPPCHILL_BOT_COMMAND`.
